...
namespace horizon {
	...
	bool ToolHelperMerge::merge_bus_net(class Net *net, class Bus *bus, class Net *net_other, class Bus *bus_other) {
		...
		else if(net && net_other && net->uuid != net_other->uuid) { //both have nets, need to merge nets
		...
		for(const auto &it:core.r->selection) {
			if(it.type == ObjectType::JUNCTION) {
				...
				/*for(auto &it_other: core.c->get_sheet()->net_lines) {
					...
					if(!core.r->selection.count(SelectableRef(line->uuid, ObjectType::LINE_NET))) {  //not in selection
						if(line->coord_on_line(ju->position)) { //need to split line
							std::cout << "maybe split net line" <<std::endl;
							auto do_merge = merge_bus_net(ju->net, ju->bus, line->net, line->bus);
							if(do_merge) {
								core.c->get_sheet()->split_line_net(line, ju);
							}
						}
					}
				}*/
