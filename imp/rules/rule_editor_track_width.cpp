...
namespace horizon {
	void RuleEditorTrackWidth::populate() {
		...
		builder = Gtk::Builder::create_from_resource("/net/carrotIndustries/horizon/imp/rules/rule_editor_track_width.ui");
		Gtk::Box *editor;
		...
		for(const auto &it: layers) {
			...
			if(la.copper) {
				...
				sp_min->signal_value_changed().connect([this, i, sp_min] {
					rule2->widths.at(i).min = sp_min->get_value_as_int();
				});
				...
				sp_default->signal_value_changed().connect([this, i, sp_default] {
					rule2->widths.at(i).def = sp_default->get_value_as_int();
				});
				...
				sp_max->signal_value_changed().connect([this, i, sp_max] {
					rule2->widths.at(i).max = sp_max->get_value_as_int();
				});
