...
namespace horizon {
	...
	void DragSelection::Line::realize() {
		program = gl_create_program_from_resource("/net/carrotIndustries/horizon/canvas/shaders/selection-line-vertex.glsl", "/net/carrotIndustries/horizon/canvas/shaders/selection-line-fragment.glsl", "/net/carrotIndustries/horizon/canvas/shaders/selection-line-geometry.glsl");
		create_vao();
		...
		for(auto &it: ca->selectables.items) {
			...
			if(ca->selection_filter.can_select(ca->selectables.items_ref[i])) {
				if(ca->selection_qualifier == CanvasGL::SelectionQualifier::INCLUDE_ORIGIN) {
					...
					if(std::all_of(corners.begin(), corners.end(), [xmin, xmax, ymin, ymax] (const auto &a) {return a.x > xmin && a.x < xmax && a.y > ymin && a.y < ymax;})) {
						it.set_flag(Selectable::Flag::PRELIGHT, true);
					}
		...
		for(auto &it: ca->selectables.items) {
			...
			if(ca->selection_filter.can_select(ca->selectables.items_ref[i])) {
				if(ca->selection_tool == CanvasGL::SelectionTool::PAINT) {
					...
					else if(ca->selection_qualifier == CanvasGL::SelectionQualifier::INCLUDE_BOX || ca->selection_qualifier == CanvasGL::SelectionQualifier::TOUCH_BOX) {
						ClipperLib::Path sel(4);
						auto corners = it.get_corners();
						for(size_t j = 0; j<4; j++) {
							sel.at(j) = ClipperLib::IntPoint(corners[j].x, corners[j].y);
