...
static const struct footag_typeinfo info = {
        .type   = FOOTAG_TYPE_PQFN,
        .name   = "PQFN",
        .brief  = "Quad flat no-lead with pullback",
};

/* Silicon Labs Si5332E/F/G/H */
static const struct footag_param temp[] = {
        PARAM_TOPIC("Body"),
        PARAM_T(BODY_L,  "Length",      "D",  5.00, 0.10),
        PARAM_T(BODY_W,  "Width",       "E",  5.00, 0.10),
        PARAM_I(BODY_N,  "Pins (1...)", "-",  8, 1, 1, 256),
        PARAM_I(BODY_Nx, "Pins (vert)", "-",  8, 1, 1, 256),

        PARAM_TOPIC("Lead"),
        PARAM_L(LEAD_P,  "Pitch",       "e",  0.50),
        PARAM_T(LEAD_L,  "Length",      "L",  0.32, 0.42),
        PARAM_T(LEAD_W,  "Width",       "b",  0.20, 0.30),
        PARAM_L(LEAD_PB, "Pullback",    "L1", 0.10),
...
static int calc(struct footag_ctx *ctx)
{
        ...
        const double pullback = GETID(ctx, LEAD_PB)->l;
        ...
        ret = ipcb_get_pson(&spec_row, &comp_row, &attr, pullback);
        if (ret) { return ret; }
        ret = ipcb_get_pson(&spec_col, &comp_col, &attr, pullback);
        if (ret) { return ret; }
...
const struct footag_op footag_op_pqfn = {
