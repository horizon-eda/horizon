...
void ToolMapPort::create_port(const UUID &uu)
{
    ...
    port = &doc.o->get_block_symbol()
                    ...
                    .first->second;
    port->name = doc.o->get_block_symbol().block->get_net_name(uu);
    port->direction = doc.o->get_block_symbol().block->nets.at(uu).port_direction;
...
ToolResponse ToolMapPort::begin(const ToolArgs &args)
{
    ...
    for (const auto &it : args.selection) {
        if (it.type == ObjectType::BLOCK_SYMBOL_PORT) {
            if (doc.o->get_block_symbol().ports.count(it.uuid) == 0) { // unplaced port
