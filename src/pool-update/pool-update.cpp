...
namespace horizon {
	...
	static void update_entities(Pool &pool, const std::string &directory, pool_update_cb_t status_cb, const std::string &prefix="") {
		Glib::Dir dir(directory);
		for(const auto &it: dir) {
			std::string filename = Glib::build_filename(directory, it);
			if(endswith(it, ".json")) {
				try {
					...
					SQLite::Query q(pool.db, "INSERT INTO entities (uuid, name, manufacturer, filename, n_gates, prefix) VALUES ($uuid, $name, $manufacturer, $filename, $n_gates, $prefix)");
					...
					for(const auto &it_tag: entity.tags) {
						SQLite::Query q2(pool.db, "INSERT into tags (tag, uuid, type) VALUES ($tag, $uuid, 'entity')");
	...
	static void update_symbols(Pool &pool, const std::string &directory, pool_update_cb_t status_cb, const std::string &prefix="") {
		Glib::Dir dir(directory);
		for(const auto &it: dir) {
			std::string filename = Glib::build_filename(directory, it);
			if(endswith(it, ".json")) {
				try {
					...
					SQLite::Query q(pool.db, "INSERT INTO symbols (uuid, name, filename, unit) VALUES ($uuid, $name, $filename, $unit)");
	...
	static void update_package_node(Pool &pool, const PoolUpdateNode &node, std::set<UUID> &visited, pool_update_cb_t status_cb) {
		...
		try {
			if(filename.size()) {
				...
				SQLite::Query q(pool.db, "INSERT INTO packages (uuid, name, manufacturer, filename, n_pads, alternate_for) VALUES ($uuid, $name, $manufacturer, $filename, $n_pads, $alt_for)");
				...
				for(const auto &it_tag: package.tags) {
					SQLite::Query q2(pool.db, "INSERT into tags (tag, uuid, type) VALUES ($tag, $uuid, 'package')");
					q2.bind("$uuid", package.uuid);
					q2.bind("$tag", it_tag);
					q2.step();
				}
				for(const auto &it_model: package.models) {
					SQLite::Query q2(pool.db, "INSERT INTO models (package_uuid, model_uuid, model_filename) VALUES (?, ?, ?)");
	...
	static void update_part_node(Pool &pool, const PoolUpdateNode &node, std::set<UUID> &visited, pool_update_cb_t status_cb) {
		if(visited.count(node.uuid)) {
			status_cb(PoolUpdateStatus::FILE_ERROR, node.filename, "detected cycle");
			return;
		}
		...
		try {
			if(filename.size()) {
				...
				SQLite::Query q(pool.db, "INSERT INTO parts (uuid, MPN, manufacturer, entity, package, description, filename) VALUES ($uuid, $MPN, $manufacturer, $entity, $package, $description, $filename)");
				...
				for(const auto &it_tag: part.get_tags()) {
					SQLite::Query q2(pool.db, "INSERT into tags (tag, uuid, type) VALUES ($tag, $uuid, 'part')");
