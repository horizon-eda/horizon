...
namespace horizon {
	...
	PoolUpdateNode::PoolUpdateNode(const UUID &u, const std::string &fn,const std::set<UUID> &deps): uuid(u), filename(fn), dependencies(deps) {
	...
	void PoolUpdateGraph::add_node(const UUID &uu, const std::string &filename, const std::set<UUID> &dependencies) {
		...
		nodes.emplace(std::piecewise_construct, std::forward_as_tuple(uu), std::forward_as_tuple(uu, filename, dependencies));
	}
