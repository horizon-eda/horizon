...
namespace horizon {
	...
	WindowStateStore::WindowStateStore(Gtk::Window *w,const std::string &wn): db(Glib::build_filename(get_config_dir(), "window_state.db"), SQLITE_OPEN_READWRITE|SQLITE_OPEN_CREATE), window_name(wn), win(w) {
		...
		{
			SQLite::Query q(db, "SELECT width, height, x, y, maximized FROM window_state WHERE window=?");
	...
	void WindowStateStore::save(const std::string &w, const WindowState &ws) {
		SQLite::Query q(db, "INSERT OR REPLACE INTO window_state (window, width, height, x, y, maximized) VALUES (?, ?, ?, ?, ?, ?)");
		q.bind(1, w);
		q.bind(2, ws.width);
		q.bind(3, ws.height);
		q.bind(4, ws.x);
