...
namespace horizon {
ImplicitPreferences &ImplicitPreferences::get()
{
    static auto x = ImplicitPreferences();
    return x;
}

static const int min_user_version = 1;

ImplicitPreferences::ImplicitPreferences()
    : db_path(Glib::build_filename(get_config_dir(), "implicit_prefs.db")),
...
{
    {
        ...
        if (user_version < min_user_version) {
            ...
            if (Glib::file_test(old_db_path, Glib::FILE_TEST_IS_REGULAR)) {
                {
