...
{
    program = gl_create_program_from_resource("/net/carrotIndustries/horizon/lib/canvas3d/shaders/cover-vertex.glsl",
...
void CoverRenderer::push()
{
    ...
    for (const auto &it : ca->layers) {
        glBufferSubData(GL_ARRAY_BUFFER, ofs * sizeof(Canvas3D::Layer3D::Vertex),
                        it.second.tris.size() * sizeof(Canvas3D::Layer3D::Vertex), it.second.tris.data());
