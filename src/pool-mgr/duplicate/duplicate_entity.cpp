...
namespace horizon {
	DuplicateEntityWidget::DuplicateEntityWidget(Pool *p, const UUID &entity_uuid,  Gtk::Box *ubox, bool optional, DuplicateWindow *w): Gtk::Box(Gtk::ORIENTATION_VERTICAL, 10), pool(p), entity(pool->get_entity(entity_uuid)), win(w) {
		...
		location_entry->set_filename(DuplicateUnitWidget::insert_filename(pool->get_filename(ObjectType::ENTITY, entity->uuid), "-copy"));
