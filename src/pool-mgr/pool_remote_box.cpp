...
namespace horizon {
	...
	SignInDialog::SignInDialog(Gtk::Window *parent) : Gtk::Dialog("Sign in with GitHub", *parent, Gtk::DialogFlags::DIALOG_MODAL|Gtk::DialogFlags::DIALOG_USE_HEADER_BAR) {
		...
		la->set_markup("Your credentials will not be saved.\nDon't have a GitHub account? <a href='https://github.com/join'>Sign up</a>");
	...
	ConfirmPrDialog::ConfirmPrDialog(Gtk::Window *parent) : Gtk::Dialog("Confirm Pull Request", *parent, Gtk::DialogFlags::DIALOG_MODAL|Gtk::DialogFlags::DIALOG_USE_HEADER_BAR) {
		add_button("Cancel", Gtk::ResponseType::RESPONSE_CANCEL);
		...
		auto la = Gtk::manage(new Gtk::Label());
		la->set_markup("This information will be used for the commit and will be visible publicly.");
	...
	void PoolRemoteBox::create_pr_thread() {
		try {
...

			{
				...
				if(git_commit_create(&new_commit_oid, repo, "HEAD", signature, signature, "UTF-8", pr_title.c_str(), tree, 1, &parent_commit_p) != 0) {
