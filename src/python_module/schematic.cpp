...
{
    auto self = reinterpret_cast<PySchematic *>(pself);
    auto settings = self->schematic->blocks.get_top_block_item().schematic.pdf_export_settings.serialize_schematic();
...
static PyObject *PySchematic_export_pdf(PyObject *pself, PyObject *args)
{
    ...
    try {
        ...
        horizon::export_pdf(self->schematic->blocks.get_top_block_item().schematic, settings, nullptr);
    }
...
{
    auto self = reinterpret_cast<PySchematic *>(pself);
    auto settings = self->schematic->blocks.get_top_block_item().block.bom_export_settings.serialize();
    return py_from_json(settings);
}
...
static PyObject *PySchematic_export_bom(PyObject *pself, PyObject *args)
{
    ...
    try {
        ...
        horizon::export_BOM(settings.output_filename, self->schematic->blocks.get_top_block_item().block, settings);
    }
    catch (const std::exception &e) {
        PyErr_SetString(PyExc_IOError, e.what());
        return NULL;
