...
TreeWriterArchive::TreeWriterArchive(const fs::path &filename, const fs::path &pre, Type ty) : prefix(pre), type(ty)
{
...
std::ostream &TreeWriterArchive::create_file_internal(const fs::path &path)
{
    ...
    const auto path2 = prefix / path;
    const auto parent = path2.parent_path();
    mkdir_recursive(parent);
    ...
    archive_entry_clear(ar_entry);
    archive_entry_set_pathname(ar_entry, path2.u8string().c_str());
