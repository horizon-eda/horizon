...
SheetBox::SheetBox(CoreSchematic &c) : Gtk::Box(Gtk::Orientation::ORIENTATION_VERTICAL, 0), core(c)
{
    ...
    {
        auto tbo = Gtk::manage(new Gtk::ToolButton());
        tbo->set_icon_name("view-more-symbolic");
...
void SheetBox::selection_changed()
{
    ...
    if (it) {
        ...
        if (sheet_uu == UUID()) {
            ...
            remove_button->set_sensitive(sh.can_be_removed());
...
void SheetBox::update_highlights(const UUID &uu, bool v)
{
    for (const auto &it : store->children()) {
        Gtk::TreeModel::Row row = *it;
        if (row[tree_columns.sheet] == uu) {
            row[tree_columns.has_highlights] = v;
            break;
        }
    }
}
...
void SheetBox::update()
{
    ...
    {
        const auto s = core.get_current_schematic()->sheets.size() > 1;
