...
namespace horizon {
	...
	static GLuint create_vao (GLuint program, GLuint &vbo_out, GLuint &ebo_out, GLuint &vbo_instance_out) {
		...
		glVertexAttribPointer (position_index, 3, GL_FLOAT, GL_FALSE,
							 sizeof(Canvas3D::FaceVertex),
							 0);
		glEnableVertexAttribArray (color_index);
		glVertexAttribPointer (color_index, 3, GL_UNSIGNED_BYTE, GL_TRUE,
							 sizeof(Canvas3D::FaceVertex),
		...
		Canvas3D::ModelTransform ivertices[] = {
		//   Position
				{0,0,0,0},
				{20,20,32768,0}
		};
	...
	void FaceRenderer::realize() {
		program = gl_create_program_from_resource("/net/carrotIndustries/horizon/canvas3d/shaders/face-vertex.glsl", "/net/carrotIndustries/horizon/canvas3d/shaders/face-fragment.glsl", "/net/carrotIndustries/horizon/canvas3d/shaders/face-geometry.glsl");
	...
	void FaceRenderer::push() {
		if(ca->models_loading_mutex.try_lock()) {
			...
			glBufferData (GL_ARRAY_BUFFER, sizeof (Canvas3D::FaceVertex)*n_vertices, ca->face_vertex_buffer.data(), GL_STATIC_DRAW);

			glBindBuffer (GL_ELEMENT_ARRAY_BUFFER, ebo);
			glBufferData (GL_ELEMENT_ARRAY_BUFFER, sizeof (unsigned int)*n_idx, ca->face_index_buffer.data(), GL_STATIC_DRAW);
			ca->models_loading_mutex.unlock();
		}

		glBindBuffer (GL_ARRAY_BUFFER, vbo_instance);
		glBufferData (GL_ARRAY_BUFFER, sizeof (Canvas3D::ModelTransform)*ca->package_transforms.size(), ca->package_transforms.data(), GL_STATIC_DRAW);
	}
	...
	void FaceRenderer::render() {
		if(ca->models_loading_mutex.try_lock()) {
			...
			glUniform1f(z_top_loc, ca->get_layer_offset(BoardLayers::TOP_COPPER)+ca->layers.at(BoardLayers::TOP_COPPER).thickness);
